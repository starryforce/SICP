...
#|
(define (eval-let alol exp)
  (eval-1 (substitute exp   ; the body
                      (map (lambda (x) (car x)) alol)            ; the formal parameters
                      (map (lambda (x) (eval-1 (cadr x))) alol)  ; the actual arguments
                      '())))
|#

(define (eval-let exp)
  (eval-1 (cons (list 'lambda
                      (let-keys exp)
                      (let-body exp))
                (let-values exp))))